...
dein Tennisteam-Zugang ist bereit.

Admin-Link:
$adminUrl

Bitte den Link vertraulich behandeln und nicht oeffentlich weitergeben.

Empfohlener Start:
1. Teamdaten kurz pruefen
2. Spieler anlegen
3. Saison anlegen
4. Spieler-Links im Admin verteilen

Falls du Fragen hast oder Hilfe beim Setup brauchst, melde dich einfach.

Viele Gruesse
"@

$sqlPath = Join-Path $outDir "${timestamp}_${slug}_setup.sql"
$mailPath = Join-Path $outDir "${timestamp}_${slug}_admin_mail.txt"

Set-Content -Path $sqlPath -Value $sql -Encoding utf8
Set-Content -Path $mailPath -Value $mail -Encoding utf8

Write-Host ""
Write-Host "== Neues Tennisteam vorbereitet ==" -ForegroundColor Cyan
...
Write-Host "3. Mailtext versenden"
