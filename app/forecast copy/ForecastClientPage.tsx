...
import type { ForecastPoint, ForecastYearRow } from "@/lib/forecast";
...
function toForecastPoints(rows: ForecastYearRow[]): ForecastPoint[] {
  return rows.map((r, i) => ({
    yearIndex: i,
    age: 0, // minimal: damit es kompiliert (ForecastChart erwartet age)
...
export default function ForecastPage() {
  ...
  const [startAvail, setStartAvail] = useState<StartAvail>({
    liq: 0,
    kfr: 0,
    available: 0,
  });
  ...
  useEffect(() => {
    ...
    async function run() {
      try {
        ...
        const self = profile.household?.persons?.find((p: any) => p.role === "self");
        setRetireAtAge(self?.retireAtAge ?? 65);

        // NEW: Ausgangslage (UI)
        setStartAvail(deriveStartAvailable(profile as any));
        setEmptyMsg("");

        const out = computeForecastFromProfileV2(profile);
        if (cancelled) return;

        // nur damit es kompiliert: Chart erwartet ForecastPoint[]
        setData(out.points ?? []);

        // echte Startwerte aus ForecastOut
        setStartAvail({
        ...
        // falls out.breakdowns existiert:
        setBreakdowns((out as any).breakdowns ?? []);
      } catch (e) {
        ...
        <>
          ...
          <div className="h-[320px]">
            <ForecastChart
              ...
              onSelectAge={(age: number) => setOpenAge(age)}
            />
          </div>
  ...
  return (
    <div className="rounded-2xl border border-slate-800 bg-slate-950/40 p-4">
      ...
      <div className="space-y-2">
        {shown.map((b) => {
          const eventsNet = (b.totals as any).eventsNet ?? 0;
              ...
              <summary className="cursor-pointer list-none">
                <div className="flex flex-wrap items-center justify-between gap-3">
                  ...
                  <div className="text-xs text-slate-400 text-right">
                    ...
                    Basis: <span className="text-slate-200">{formatCHF(baseNet)}</span>
                    <span className="text-slate-500"> · </span>
                    Events: <span className="text-slate-200">{formatCHF(eventsNet)}</span>
                    <span className="text-slate-500"> · </span>
                    Ende:{" "}
                    <span className="text-slate-200">{b.wealthEnd.toLocaleString("de-CH")} CHF</span>
                  </div>
                </div>
              </summary>
