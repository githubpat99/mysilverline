# Deploy Silverline-API (PHP) per rsync/scp
# Ziel: wp-content/plugins/silverline-api/
# Auf Windows: Nutzt WSL oder Git Bash falls vorhanden (rsync), sonst scp
#
# Nutzung:
#   .\deploy-Backend.ps1         = Deploy ausführen
#   .\deploy-Backend.ps1 --dry   = Dry-Run (zeigt nur, was übertragen würde)

$ErrorActionPreference = "Stop"
$Host.UI.RawUI.WindowTitle = "Silverline API Deploy"
...
Write-Host "== Deploy Silverline API (PHP) ==" -ForegroundColor Cyan
Write-Host "$localDir -> ${sshUser}@${sshHost}:${remoteDir}" -ForegroundColor Gray
...
# 3. Fallback: scp (ohne rsync - kopiert nur PHP-API-Dateien)
$scp = Get-Command scp -ErrorAction SilentlyContinue
if ($scp) {
    Write-Host "rsync nicht gefunden. Nutze scp..." -ForegroundColor Yellow
    $files = @("silverline-api.php")
    $copied = 0
    foreach ($f in $files) {
    ...
    if ($copied -eq 0) {
        Write-Host "Fehler: silverline-api.php nicht gefunden in $localDir" -ForegroundColor Red
        exit 1
    }
