...
function sl_sync_link_user(WP_REST_Request $req) {
  ...
  $exists = $wpdb->get_var($wpdb->prepare("SELECT 1 FROM {$table} WHERE wp_user_id = %d LIMIT 1", $uid));

  if ($exists) {
    $wpdb->update($table, ['local_user_id' => $local_user_id, 'updated_at' => current_time('mysql', 1)], ['wp_user_id' => $uid], ['%s', '%s'], ['%d']);
  } else {
    $wpdb->insert($table, ['wp_user_id' => $uid, 'local_user_id' => $local_user_id, 'updated_at' => current_time('mysql', 1)], ['%d', '%s', '%s']);
  }
...
function sl_sync_pull(WP_REST_Request $req) {
  ...
  return new WP_REST_Response(['ok' => true, 'profile' => $profile, 'positions' => $positions], 200);
}
